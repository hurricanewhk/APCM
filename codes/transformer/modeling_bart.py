...
class BartEncoder(BartPreTrainedModel):
    ...
    def forward(
        ...
        ##
        # If token_type_ids are not provided, set to zeros (default to one segment)
        ...
        if token_type_ids is None:
            token_type_ids = torch.zeros(input.shape, dtype=torch.long, device=inputs_embeds.device)
         # Compute token type embeddings
        token_type_embeddings = self.token_type_embeddings(token_type_ids)

        embed_pos = self.embed_positions(input)
        embed_pos = embed_pos.to(inputs_embeds.device)

        hidden_states = inputs_embeds + embed_pos + token_type_embeddings ##
