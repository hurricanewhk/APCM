...
for infer_idx, infer_input_file in enumerate(args.infer_input_file):
    ...
    for batch, posts, references, sample_ids in infer_dataloader:
        ...
        if 'other_res' in batch:
            ...
            if add_acc:
                ...
                for k, v in batch_other_res['acc_map'].items():
                    ...
                    if f'{v}_top2' in encoded_info:
               
                        encoded_info[f'{v}_top2'] = encoded_info[f'{v}_top2'].tolist()
                    if f'{v}_top3' in encoded_info:
                        encoded_info[f'{v}_top3'] = encoded_info[f'{v}_top3'].tolist()
                    if f'{v}_top4' in encoded_info:
                        encoded_info[f'{v}_top4'] = encoded_info[f'{v}_top4'].tolist()
                    if f'{v}_top5' in encoded_info:
                        encoded_info[f'{v}_top5'] = encoded_info[f'{v}_top5'].tolist()
                    ...
                    if f'{v}_top2' in encoded_info:
                        ...
                        if f'{v}_top2' not in other_res['acc']:
                            other_res['acc'][f'{v}_top2'] = []
                        other_res['acc'][f'{v}_top2'].extend(encoded_info[f'{v}_top2'])
                    if f'{v}_top3' in encoded_info:
                        if f'{v}_top3' not in other_res['acc']:
                            other_res['acc'][f'{v}_top3'] = []
                        other_res['acc'][f'{v}_top3'].extend(encoded_info[f'{v}_top3'])
                    if f'{v}_top4' in encoded_info:
                        if f'{v}_top4' not in other_res['acc']:
                            other_res['acc'][f'{v}_top4'] = []
                        other_res['acc'][f'{v}_top4'].extend(encoded_info[f'{v}_top4'])
                    if f'{v}_top5' in encoded_info:
                        if f'{v}_top5' not in other_res['acc']:
                            other_res['acc'][f'{v}_top5'] = []
                        other_res['acc'][f'{v}_top5'].extend(encoded_info[f'{v}_top5'])
                ...
                if add_acc:
                    for k, v in batch_other_res['acc_map'].items():
                        ...
                        if f'{v}_top2' in encoded_info:
                            ...
                            other_res_to_append[f'{v}_top2'] = encoded_info[f'{v}_top2'][idx]
                        if f'{v}_top3' in encoded_info:
                            other_res_to_append[f'{v}_top3'] = encoded_info[f'{v}_top3'][idx]
                        if f'{v}_top4' in encoded_info:
                            other_res_to_append[f'{v}_top4'] = encoded_info[f'{v}_top4'][idx]
                        if f'{v}_top5' in encoded_info:
                            other_res_to_append[f'{v}_top5'] = encoded_info[f'{v}_top5'][idx]
                        if f'{v}_dist' in encoded_info:
        ...
        #raise EOFError
    # 计算 BERTScore
    P, R, F1 = score(gens, refs,model_type="distilbert-base-uncased",lang="en", verbose=True)
    print(f"items: {len(F1.tolist())}")
    ...
    for item in res:
        ...
        if sample_ids not in dialog:
            dialog[sample_ids] = []
        dialog[sample_ids].append("User: " + item['post'])
        dialog[sample_ids].append("System: "+ item['pred_strat_id'] + item['generation'])
        dialog[sample_ids].append("Golden truth: " + item['pred_strat_id'] + item['response'])
    # selected_entries = [entry for entries in dialog.values() if len(entries) > 1 for entry in entries]
    with open(os.path.join(save_dir, f'dialog.json'), 'w') as f:
        json.dump(dialog, f, ensure_ascii=False, indent=2, sort_keys=False)
    ...
    if not args.only_encode and not args.only_generate:
        ...
        # bertscore
        metric_res_list.update({
            'bert-scores': F1.tolist()
        })
    ...
    if not args.only_generate:
        if 'acc' in other_res:
            for k, v in other_res['acc_map'].items():
                ...
                if f'{v}_top2' in other_res['acc']:
                  
                    vv_top2 = np.array(other_res['acc'][f'{v}_top2'], dtype=int)
                    metric_res[f'acc_{k}_top2'] = np.mean(np.sum((kk.reshape(-1, 1) - vv_top2) == 0, axis=-1) != 0)
                    metric_res_list[f'acc_{k}_top2'] = (np.sum((kk.reshape(-1, 1) - vv_top2) == 0, axis=-1) != 0).astype(int).tolist()
                ...
                if f'{v}_top4' in other_res['acc']:
                    vv_top4 = np.array(other_res['acc'][f'{v}_top4'], dtype=int)
                    metric_res[f'acc_{k}_top4'] = np.mean(np.sum((kk.reshape(-1, 1) - vv_top4) == 0, axis=-1) != 0)
                    metric_res_list[f'acc_{k}_top4'] = (np.sum((kk.reshape(-1, 1) - vv_top4) == 0, axis=-1) != 0).astype(int).tolist()
                if f'{v}_top5' in other_res['acc']:
                    vv_top5 = np.array(other_res['acc'][f'{v}_top5'], dtype=int)
                    metric_res[f'acc_{k}_top5'] = np.mean(np.sum((kk.reshape(-1, 1) - vv_top5) == 0, axis=-1) != 0)
                    metric_res_list[f'acc_{k}_top5'] = (np.sum((kk.reshape(-1, 1) - vv_top5) == 0, axis=-1) != 0).astype(int).tolist()
        
        metric_res['bert_score']=bert_score
        print(metric_res)
