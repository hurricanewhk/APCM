...
#         self.knowledge_input_length=len(knowledge_input_ids)

#         self.input_len = self.input_length + self.decoder_input_length


# def featurize(
#         bos, eos, persona, knowledge,
...
#             persona_input_ids = persona
#             # knowledge_input_ids=toker(knowledge).input_ids
...
#         ipt = inputs[i]
#         feat = featurize(
#             bos, eos, ipt['persona'], ipt['knowledge'],
...
#         knowledge_input_ids = my_pad_sequence([torch.tensor(f.knowledge_input_ids, dtype=torch.long) for f in features],
#                                             batch_first=True, max_len=max_len, padding_value=pad)
#         knowledge_attention_mask = my_pad_sequence(
#             [torch.tensor([1.] * f.knowledge_input_length, dtype=torch.float) for f in features],
#             batch_first=True, max_len=max_len, padding_value=0.)
