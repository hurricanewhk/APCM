...
#         self.knowledge_norm = nn.LayerNorm(self.model.config.d_model, elementwise_affine=True)  # 512
#         self.context_norm = nn.LayerNorm(self.model.config.d_model, elementwise_affine=True)  # 512
#         # self.persona_encoder = BlenderbotSmallForConditionalGeneration(config).model.encoder
#         # self.knowledge_encoder = BlenderbotSmallForConditionalGeneration(config).model.encoder
#         # self.context_encoder = BlenderbotSmallForConditionalGeneration(config).model.encoder
#         # self.persona_context_w = nn.Parameter(torch.tensor([0.5, 0.5]))
#         self.persona_context_w = nn.Parameter(torch.tensor([1 / 3, 1 / 3, 1 / 6, 1 / 6]))
#         self.my_past = None
...
#             #     print(f"persona_input_ids shape: {persona_input_ids.shape}")
#             # if persona_attention_mask is not None:
#             #     print(f"persona_attention_mask shape: {persona_attention_mask.shape}")
            
#             # if knowledge_input_ids is not None:
#             #     print(f"knowledge_input_ids shape: {knowledge_input_ids.shape}")
#             # if knowledge_attention_mask is not None:
#             #     print(f"knowledge_attention_mask shape: {knowledge_attention_mask.shape}")
...
#             knowledge_encoder_outputs = self.model.encoder(
...
#                                    zip(encoder_outputs[0], persona_encoder_outputs[0])])
#             knowledge = torch.stack([torch.matmul(torch.softmax(torch.matmul(i, j.t()), dim=-1), j) for i, j in
#                                    zip(encoder_outputs[0], knowledge_encoder_outputs[0])])
...
#             knowledge=self.knowledge_norm(encoder_outputs.last_hidden_state+knowledge)
#             # 归一化权重
...
#             encoder_outputs.last_hidden_state = w1 * encoder_outputs.last_hidden_state + w2 * context + w3 * persona + w4 *knowledge
